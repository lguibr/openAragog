...
let filteredDataBase = inputData.map(filterByCitiesCod)
...
let selectedDataBase = filteredDataBase
...
const scrapeSelected = ()=> { Promise.all(selectedDataBase.map(getHTML))
    .then(() => {
        reportScrape()
    }).then(() => {
        writeData()
